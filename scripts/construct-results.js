...
const finalizeData = (sectionResults, categoryResults, nomineeResults) => {
  ...
  categoryResults.forEach(category => {
    ...
    if (category.fields) {
      if (typeof category.fields === "string") newCategory.fields = JSON.parse(category.fields)
      else newCategory.fields = category.fields
    }
    if (category.collection) newCategory.collection = category.collection
    if (!SECTION_ORDER.includes(category.section)) SECTION_ORDER.push(category.section)
  ...
  nomineeResults.forEach(nominee => {
    let nomineeData = typeof nominee.data === "string" ? JSON.parse(nominee.data) : nominee.data
    if (!nomineeData) return console.log(`Nominee ${nominee.idNomination} has no data. This shouldn't happen.`, nominee)
    let newNominee = {
  ...
  Object.entries(CATEGORIES_IN_SECTIONS).forEach(([sectionName, sectionCategories]) => {
...
const selectNominees = (sections, categories) => {
  ...
  console.log(`\nGetting nominee data... ${approvedOnly ? "Approved entries only" : "Unvetted entries included"}`)
