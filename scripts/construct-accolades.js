...
const readFile = () => {
  ...
  if (fs.existsSync(preferredPath))
    return [preferredPath, fs.readFileSync(preferredPath)]
  if (fs.existsSync(resultsPath))
    return [resultsPath, fs.readFileSync(resultsPath)]
}
...
if (!results) {
  let path = process.env.JSON_PATH
  console.log(
    `Failed to read results data! I found no file at${
      path ? ` your specified path ("${path}"), or at` : ''
    } "${preferredPath}" or "${resultsPath}"! Please make sure a valid JSON file exists. You might want to run ./construct-results.js first.`
  )
  process.exit()
}
...
const accoladeName =
  process.env.ACCOLADE_NAME || `Cauldron Awards ${currentYear}`
...
const addNomineeToData = (type, length, category, nominee) => {
  let creator =
    nominee.author || nominee.artist || nominee.owner || nominee.name
  if (!creator) return
...
categories.forEach(category => {
  ...
  console.log(
    `[${golds.length} gold, ${silvers.length} silver, ${bronzes.length} bronze] — category ${category.id}: ${category.title}`
  )

  golds.forEach(nominee =>
    addNomineeToData('gold', golds.length, category, nominee)
  )
  silvers.forEach(nominee =>
    addNomineeToData('silver', silvers.length, category, nominee)
  )
  bronzes.forEach(nominee =>
    addNomineeToData('bronze', bronzes.length, category, nominee)
  )
})
...
for (const user in DATA) {
  ...
  payloadData.forEach((userAccolade, index) => {
    ...
    let filtered = payloadData.filter(
      (current, i) => index !== i && userAccolade === current
    )
