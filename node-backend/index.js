...
app.use(
  '/images',
  express.static(path.join(__dirname, '../build/static/images/'))
)
...
app.get('/login', (req, res) => {
  ...
  res.redirect(
    `https://discordapp.com/api/oauth2/authorize?client_id=${CLIENT_ID}&scope=identify+guilds&response_type=code&redirect_uri=${redirect}`
  )
})
...
app.get('/authenticated', (req, res) => {
  ...
  axios({
    ...
    .then(async tokenResponse => {
      console.log(
        `${new Date().toUTCString()} - Token received: ${
      ...
      userData.is_cauldron_member =
        guildsResponse.data.filter(g => g.id === GUILD_ID).length === 1
